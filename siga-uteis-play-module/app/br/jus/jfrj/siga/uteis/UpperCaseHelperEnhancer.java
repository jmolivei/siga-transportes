...
public class UpperCaseHelperEnhancer extends Enhancer {
	...
	public void enhanceThisClass(ApplicationClass applicationClass) throws Exception {
		...
		/** Pesquisando para verificar se classe é derivada de GenericModel ou Model e se
		 *  tem algum atributo marcado com a annotation Sequence.
		 */
		if (!ctClass.subtypeOf(classPool.get("play.db.jpa.GenericModel")) && !ctClass.subtypeOf(classPool.get("play.db.jpa.Model"))) {
		...
		for (CtMethod ctMethod : ctClass.getMethods()) {
			//	Logger.info("Nome do metodo ja adicionado: " + ctMethod.getName());
		...
		for (CtField ctField : ctClass.getFields()) {
			if (hasAnnotation(ctField, "br.jus.jfrj.siga.uteis.UpperCase")) {
		...
		for (CtField ctField : fieldsToUppercase) {
			/**
			 *    Criando o mÃ©todo que converterÃ¡ todos os atributos do tipo String anotados
			 *    com @UpperCase
			 */
		...
		if (fieldsToUppercase.size() > 0) {
			...
			Logger.info("Anotaçõees geradas no método " + geraSequencia.getName() + " : " +  annotPersist.toString() + "-" + annotUpdate.toString());
