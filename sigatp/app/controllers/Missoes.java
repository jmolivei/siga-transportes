...
import models.RequisicaoVsEstado;
...
public class Missoes extends Controller {
	...
	public static void salvar(@Valid Missao missao, List<RequisicaoTransporte> requisicoesTransporte_alt,List<RequisicaoTransporte> requisicoesTransporte_ant) throws Exception {
		...
		if (novaMissao) {
			gravarAndamentos(dpPessoa, "PROGRAMADO",
					missao.requisicoesTransporte, missao,
					EstadoRequisicao.PROGRAMADA);
		} else {
	...
	private static void checarAcesso(Missao missao)  throws Exception {
			if (! AutorizacaoGI.ehAdministrador() && ! AutorizacaoGI.ehAdministradorMissao()) {
				...
				if (! AutorizacaoGI.titular().equivale(missao.condutor.dpPessoa)) {
					try {
						throw new Exception("Voce nao tem acesso a esta missão");
					} catch (Exception e) {
	...
	private static void gravarAndamentos(DpPessoa dpPessoa, String descricao,
			RequisicaoTransporte[] requisicoesTransporte, Missao missao,
			EstadoRequisicao[] estadosRequisicao) {
	...
	private static void gravarAndamentos(DpPessoa dpPessoa, String descricao,
			List<RequisicaoTransporte> requisicoesTransporte, Missao missao,
			EstadoRequisicao estadoRequisicao) {

		@SuppressWarnings("unused")
		int i = 0;
		for (RequisicaoTransporte requisicaoTransporte : requisicoesTransporte) {
			gravaAndamento(dpPessoa, descricao, missao, estadoRequisicao,
	...
	private static void gravaAndamento(DpPessoa dpPessoa, String descricao,
			Missao missao, EstadoRequisicao estadoRequisicao,
			RequisicaoTransporte requisicaoTransporte) {
		Andamento andamento = new Andamento();
		andamento.descricao = descricao;
		andamento.dataAndamento = Calendar.getInstance();
	...
	@RoleAdminMissaoComplexo
	@RoleAgente
	public static void finalizarMissao(@Valid Missao missao,
			RequisicaoVsEstado[] requisicoesVsEstados) throws Exception {
	/*	for (play.data.validation.Error error : Validation.errors()) {
		...
		RequisicaoTransporte[] requisicoes = missao.requisicoesTransporte
				.toArray(new RequisicaoTransporte[missao.requisicoesTransporte
						.size()]);
		EstadoRequisicao[] estadosRequisicao = new EstadoRequisicao[missao.requisicoesTransporte
				.size()];
		for (int i = 0; i < requisicoes.length; i++) {
			estadosRequisicao[i] = RequisicaoVsEstado.encontrarEstadoNaLista(
					requisicoesVsEstados, requisicoes[i].id);
		}

		gravarAndamentos(dpPessoa, "PELA MISSAO N." + missao.getSequence(),
				requisicoes, missao, estadosRequisicao);
		listarFiltrado(missao.estadoMissao);
	}
	...
	private static void verificarOdometroSaidaZerado(Missao missao) {
		if (missao.odometroSaidaEmKm == 0) {
			Validation.addError("odometroRetornoEmKm",
							"O valor do od&ocirc;metro de sa&iacute;da deve ser maior que zero.");
		}
	}
	...
	private static void verificarOdometroRetornoZerado(Missao missao) {
		if (missao.odometroRetornoEmKm == 0) {
			Validation.addError("odometroRetornoEmKm",
							"O valor do od&ocirc;metro de retorno deve ser maior que zero.");
		}
	}
	...
	@RoleAdminMissaoComplexo
	@RoleAgente
	public static void iniciarMissao(@Valid Missao missao,List<RequisicaoTransporte> requisicoesTransporte_alt,List<RequisicaoTransporte> requisicoesTransporte_ant, 
		...
		gravarAndamentos(dpPessoa, "PELA MISSAO N." + missao.getSequence(),
				missao.requisicoesTransporte, missao,
				EstadoRequisicao.EMATENDIMENTO);
		// listarFiltrado(missao.estadoMissao);
	...
	public static void iniciarMissaoRapido(@Valid Missao missao,List<RequisicaoTransporte> requisicoesTransporte_alt,List<RequisicaoTransporte> requisicoesTransporte_ant) throws Exception {
		...
		gravarAndamentos(dpPessoa, "PROGRAMADO POR INICIO RAPIDO PARA MISSAO NO. " + missao.getSequence(),
						missao.requisicoesTransporte, missao,
						EstadoRequisicao.PROGRAMADA);
		...
		missao.save();
		gravarAndamentos(dpPessoa, "INICIO RAPIDO PELA MISSAO N." + missao.getSequence(),
				missao.requisicoesTransporte, missao,
				EstadoRequisicao.EMATENDIMENTO);
	...
	public static void cancelarMissao(@Valid Missao missao) throws Exception {
		...
		gravarAndamentos(dpPessoa, "MISSAO CANCELADA",
				missao.requisicoesTransporte, missao,
				EstadoRequisicao.NAOATENDIDA);
	...
	private static void verificarDisponibilidadeDeVeiculo(Missao m)
			throws Exception {
	...
	private static void verificarDisponibilidadeDeCondutor(Missao m)
			throws Exception {
		Boolean condutorEstaDisponivel = Condutor.estaDisponivel(m);
		if (!condutorEstaDisponivel) {
			Validation.addError("condutor",
					"missoes.condutorEstaDisponivel.validation", m.condutor.getDadosParaExibicao());
		}
	}

	private static void verificarDatasInicialFinal(Missao m) {
		Boolean dataSaidaAntesDeDataRetorno = m.dataHoraSaida
				.before(m.dataHoraRetorno);
		if (!dataSaidaAntesDeDataRetorno) {
			Validation.addError("dataHoraRetorno",
							"missoes.dataSaidaAntesDeDataRetorno.validation");
		}
	}

	@SuppressWarnings("unchecked")
	private static void redirecionarSeErroAoSalvar(Missao missao,
		...
		if (Validation.hasErrors()) {
			...
			if (missao.requisicoesTransporte != null) {
				List<RequisicaoTransporte> requisicoesTransporte = (List<RequisicaoTransporte>) renderArgs
						.get("requisicoesTransporte");
				for (int i = 0; i < missao.requisicoesTransporte.size(); i++) {
					if (missao.requisicoesTransporte.get(i) != null) {
						RequisicaoTransporte req = RequisicaoTransporte
								.findById(missao.requisicoesTransporte.get(i).id);
	...
	private static void redirecionarSeErroAoCancelar(Missao missao)
			throws Exception {
		if (Validation.hasErrors()) {
	...
	private static void removerRequisicoesDoRenderArgs(
			List<RequisicaoTransporte> requisicoesTransporte) {
		@SuppressWarnings("unchecked")
		List<RequisicaoTransporte> requisicoes = (List<RequisicaoTransporte>) renderArgs
				.get("requisicoesTransporte");
		if (requisicoes != null) {
	...
	protected static void montarDadosParaAMissao(Missao missao)
			throws Exception {
		removerRequisicoesDoRenderArgs(missao.requisicoesTransporte);
		...
		EstadoRequisicao[] estados = new EstadoRequisicao[missao.requisicoesTransporte
				.size()];
	...
	@RoleAdminMissaoComplexo
	@RoleAgente
	public static void buscarPelaSequence(String sequence, Boolean popUp)
			throws Exception {
	...
	protected static void recuperarPelaSigla(String sequence, Boolean popUp)
			throws Exception {
	...
	@Before(priority = 200, only = { "incluir", "incluirComRequisicoes",
			"editar", "iniciar" })
	protected static void montarCombos() throws Exception {
		...
		Requisicoes.recuperarRequisicoes(
				"(dataHoraRetornoPrevisto is null or dataHoraRetornoPrevisto >= ?) and cpOrgaoUsuario = ?",
				parametros, estados);
	}
	...
	@RoleAdminMissaoComplexo
	@RoleAgente
	public static void listarVeiculosECondutoresDisponiveis(
	...
	private static List<Veiculo> listarVeiculosDisponiveis(String veiculosDisp) {
		List<Veiculo> veiculos = new ArrayList<Veiculo>();
		...
		if (itens.size() > 0) {
			for (String item : itens) {
				...
				itemVeiculo = Veiculo.findById(Long.parseLong(item));
	...
	private static List<Veiculo> listarVeiculosDisponiveis(Long idMissao,
			Long idOrgao, String dataSaida) {
		List<Veiculo> veiculos = Veiculo.listarDisponiveis(dataSaida, idMissao,
				idOrgao);
		return veiculos;
	}

	private static List<Condutor> listarCondutoresDisponiveis(Long idMissao,
			Long idOrgao, String dataSaida, PerguntaSimNao inicioRapido) throws Exception {
		List<Condutor> condutores = Condutor.listarDisponiveis(dataSaida,
				idMissao, idOrgao,inicioRapido);
		return condutores;
	}


	@SuppressWarnings("unchecked")
	protected static List<Missao> buscarPorCondutoreseEscala(
	...
	protected static void checarCategoriaCNHVeiculoCondutor(Missao missao) throws Exception {
		...
		if (missao.condutor.categoriaCNH.getDescricao().length() == 2 ) {			
			...
			if (categoriaCondutor1 != null && categoriaCondutor2 != null ){			
				if (categoriaCondutor1.compareTo(missao.veiculo.categoriaCNH) >= 0 || categoriaCondutor2.compareTo(missao.veiculo.categoriaCNH) >= 0 ){
					return ;
				}
			}						
		}else {	
