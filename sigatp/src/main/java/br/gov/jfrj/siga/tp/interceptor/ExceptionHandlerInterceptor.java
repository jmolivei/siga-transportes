 ...
 */
@RequestScoped
@Intercepts(before = PreencherDadosAutorizacaoInterceptor.class)
public class ExceptionHandlerInterceptor implements Interceptor {
	...
	private void log(Throwable e) {
		...
		if (so.getCadastrante() != null) {
			LOGGER.error(MessageFormat.format("Erro Siga-TP; Pessoa: {0}; Lotação: {1}", cadastrante.getSigla(), lotaTitular.getSigla()), e);
		}
