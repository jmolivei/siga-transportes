...
@Audited
@Table(schema = "SIGATP")
public class RequisicaoTransporte extends GenericModel implements Comparable<RequisicaoTransporte>,SequenceMethods {
	...
	public static RequisicaoTransporte buscar(String codigoRequisicao) throws Exception {
		...
		if (! Reflexao.recuperaAnotacaoField(requisicaoTransporte).equals(siglaDocumento)) {
	 	...
	 	List<RequisicaoTransporte> requisicoesTransporte =  RequisicaoTransporte.find("cpOrgaoUsuario = ? and numero = ? and YEAR(dataHora) = ?" , cpOrgaoUsuario,numero,ano).fetch();
		if (requisicoesTransporte.size() > 1) throw new Exception(Messages.get("requisicaoTransporte.codigoDuplicado.exception"));
		if (requisicoesTransporte.size() == 0 ) throw new Exception(Messages.get("requisicaoTransporte.codigoInvalido.exception"));
	...
	public static List<RequisicaoTransporte> listar(EstadoRequisicao ... estadoRequisicao) {
		...
		String qrl = "SELECT req from RequisicaoTransporte req," + 
			         "Andamento an1 " + 
			         "WHERE req.id = an1.requisicaoTransporte.id " +
			         "AND (an1.requisicaoTransporte.id, an1.dataAndamento) IN (" +
			         "SELECT an1.requisicaoTransporte.id, max(an1.dataAndamento) " +
			         "FROM Andamento an1 " + 
			         "GROUP BY an1.requisicaoTransporte.id) " +
			         "AND ("; 
	...
	public static List<RequisicaoTransporte> listarParaAgendamento(CpOrgaoUsuario cpOrgaoUsuario) throws Exception {
		...
		String qrl = "SELECT req from RequisicaoTransporte req," + 
			         "Andamento an1 " + 
			         "WHERE req.id = an1.requisicaoTransporte.id " +
			         "AND req.cpOrgaoUsuario.idOrgaoUsu = " + cpOrgaoUsuario.getIdOrgaoUsu() + " " +
			         "AND (an1.requisicaoTransporte.id, an1.dataAndamento) IN (" +
			         "SELECT an1.requisicaoTransporte.id, max(an1.dataAndamento) " +
			         "FROM Andamento an1 " + 
			         "GROUP BY an1.requisicaoTransporte.id) " + 
	...
	public Boolean cancelar(DpPessoa responsavel, String descricao) throws Exception {
		...
		if (ultimoAndamentoRequisicao == EstadoRequisicao.ABERTA ||
			ultimoAndamentoRequisicao == EstadoRequisicao.AUTORIZADA ||
			ultimoAndamentoRequisicao == EstadoRequisicao.REJEITADA ||
