...
public class NavegacaoSigaTp extends NavegacaoSiga {
	...
	private void pausar() {
		try {Thread.sleep(_TEMPO_INTERVALO_CARREGAMENTO_PAGINA_EM_SEGUNDOS * 1000);} catch (InterruptedException e) {}
	}
	...
	public void logar(String login, String senha) {
		...
		driver.findElement(By.xpath("//a[contains(text(), 'SIGA')]")).click();
		driver.findElement(By.xpath("//a[contains(text(), 'Módulos')]")).click();
		//driver.findElement(By.cssSelector("a[href*='sigatp']")).click();
	
		WebElement botao = driver.findElement(By.cssSelector("a[href*='sigatp']"));
		botao.click();
	...
	/*	WebElement menu = getPagina().getDriver().findElement(By.id("menuRequisicoes"));
		pausar();
		menu.click();
		
		WebElement menuHoverLink = getPagina().getDriver().findElement(By.id("menuRequisicoesIncluir"));
		Locatable hoverItem = (Locatable) menuHoverLink; 
		Mouse mouse = ((HasInputDevices) getPagina().getDriver()).getMouse(); 
